...
public class AuthQueryServiceImpl implements AuthQueryService {
    ...
    public AuthResDTO.LoginDTO kakaoLogin(AuthReqDTO.@Valid KakaoLoginDTO dto) {
        log.info("=== 카카오 JWT 토큰 검증 시작 ===");

        String jwtToken = dto.kakaoAccessToken();

        // 1. JWT 토큰 검증
        if (!jwtUtil.isValid(jwtToken)) {
            log.error("JWT 토큰 검증 실패: 유효하지 않은 토큰");
        ...
        // 5. 새로운 JWT 토큰 생성 (createAccessToken 사용 - role 포함)
        String newAccessToken = jwtUtil.createAccessToken(userDetails);
        String newRefreshToken = jwtUtil.createRefreshToken(userDetails);
        ...
        log.info("JWT 토큰 갱신 완료: email={}", email);
